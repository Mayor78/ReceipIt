...
const QuickStats = ({ stats, formatNaira, onDateRangeChange }) => {
  ...
  const safeStats = {
    ...
    largestReceipt: stats?.largestReceipt || 0,
  ...
  const statsCards = [
    ...
    {
      title: 'Gross_Value',
      value: formatNaira ? formatNaira(safeStats.totalAmount) : `₦${safeStats.totalAmount.toLocaleString()}`,
      icon: DollarSign,
    ...
    {
      title: 'Monthly_Log',
      value: formatNaira ? formatNaira(safeStats.monthlyTotal) : `₦${safeStats.monthlyTotal.toLocaleString()}`,
      icon: Calendar,
      glow: 'shadow-purple-500/10',
      text: 'text-purple-500',
      border: 'border-purple-500/20',
      subtext: `${safeStats.monthlyCount} entries`
    },
    {
      title: 'Mean_Velocity',
      value: formatNaira ? formatNaira(safeStats.averageAmount) : `₦${safeStats.averageAmount.toLocaleString()}`,
      icon: TrendingUp,
    ...
    {
      title: 'Peak_Record',
      value: formatNaira ? formatNaira(safeStats.largestReceipt) : `₦${safeStats.largestReceipt.toLocaleString()}`,
      icon: Award,
        ...
        {statsCards.map((stat, index) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                if (stat.title === 'Monthly_Log' && onDateRangeChange) {
                  const now = new Date();
